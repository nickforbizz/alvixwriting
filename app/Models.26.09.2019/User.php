<?php

namespace App\Models;

use Illuminate\Database\Eloquent\Model;
use Illuminate\Notifications\Notifiable;
use App\Notifications\ResetPasswordWriter;
use Illuminate\Contracts\Auth\CanResetPassword;
use Illuminate\Foundation\Auth\User as authenticatable;
// use Illuminate\Auth\Authenticatable as AuthenticableTrait;

/**
 * @property int $id
 * @property int $national_id
 * @property string $email
 * @property string $password
 * @property string $username
 * @property string $fname
 * @property string $lname
 * @property string $sname
 * @property int $age
 * @property string $gender
 * @property string $email_verified_at
 * @property int $mobile
 * @property string $roles
 * @property string $address
 * @property string $bio
 * @property string $remember_token
 * @property int $active
 * @property int $status
 * @property string $created_at
 * @property string $updated_at
 * @property string $deleted_at
 * @property AdminMediaProfile[] $adminMediaProfiles
 * @property AssgPendingPayment[] $assgPendingPayments
 * @property ChatsAdmin[] $chatsAdmins
 * @property ChatsUser[] $chatsUsers
 * @property Onprogressassignment[] $onprogressassignments
 * @property Onreviewassignment[] $onreviewassignments
 * @property Onrevisionassignment[] $onrevisionassignments
 * @property RejectedAssg[] $rejectedAssgs
 * @property Smse[] $smses
 * @property UserFeedback[] $userFeedbacks
 * @property UserRating[] $userRatings
 * @property WriterMediaFilesAssg[] $writerMediaFilesAssgs
 * @property WriterMediaProfile[] $writerMediaProfiles
 */
class User extends Authenticatable implements CanResetPassword
{

    use Notifiable;
    /**
     * @var array
     */
    protected $table = 'usershg';

    protected $fillable = ['national_id', 'email', 'password', 'username', 'fname', 'lname', 'sname', 'age', 'gender', 'email_verified_at', 'mobile', 'roles', 'address', 'bio', 'remember_token', 'active', 'status', 'created_at', 'updated_at', 'deleted_at'];
     
    protected $hidden = [
        'password', 'remember_token',
    ];
    /**
     * @return \Illuminate\Database\Eloquent\Relations\HasMany
     */
    public function adminMediaProfiles()
    {
        return $this->hasMany('App\AdminMediaProfile');
    }

    /**
     * @return \Illuminate\Database\Eloquent\Relations\HasMany
     */
    public function assgPendingPayments()
    {
        return $this->hasMany('App\AssgPendingPayment');
    }

    /**
     * @return \Illuminate\Database\Eloquent\Relations\HasMany
     */
    public function chatsAdmins()
    {
        return $this->hasMany('App\ChatsAdmin');
    }

    /**
     * @return \Illuminate\Database\Eloquent\Relations\HasMany
     */
    public function chatsUsers()
    {
        return $this->hasMany('App\ChatsUser');
    }

    /**
     * @return \Illuminate\Database\Eloquent\Relations\HasMany
     */
    public function onprogressassignments()
    {
        return $this->hasMany('App\Onprogressassignment');
    }

    /**
     * @return \Illuminate\Database\Eloquent\Relations\HasMany
     */
    public function onreviewassignments()
    {
        return $this->hasMany('App\Onreviewassignment');
    }

    /**
     * @return \Illuminate\Database\Eloquent\Relations\HasMany
     */
    public function onrevisionassignments()
    {
        return $this->hasMany('App\Onrevisionassignment');
    }

    /**
     * @return \Illuminate\Database\Eloquent\Relations\HasMany
     */
    public function rejectedAssgs()
    {
        return $this->hasMany('App\RejectedAssg');
    }

    /**
     * @return \Illuminate\Database\Eloquent\Relations\HasMany
     */
    public function smses()
    {
        return $this->hasMany('App\Smse');
    }

    /**
     * @return \Illuminate\Database\Eloquent\Relations\HasMany
     */
    public function userFeedbacks()
    {
        return $this->hasMany('App\UserFeedback');
    }

    /**
     * @return \Illuminate\Database\Eloquent\Relations\HasMany
     */
    public function userRatings()
    {
        return $this->hasMany('App\UserRating');
    }

    /**
     * @return \Illuminate\Database\Eloquent\Relations\HasMany
     */
    public function writerMediaFilesAssgs()
    {
        return $this->hasMany('App\WriterMediaFilesAssg');
    }

    /**
     * @return \Illuminate\Database\Eloquent\Relations\HasMany
     */
    public function writerMediaProfiles()
    {
        return $this->hasMany('App\WriterMediaProfile');
    }
    /**
    * Send the password reset notification.
    *
    * @param  string  $token
    * @return void
    */
    public function sendPasswordResetNotification($token)
    {
      $this->notify(new ResetPasswordWriter($token));
    }
}



